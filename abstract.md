...
A key limitation of storage-cell 2D models is the assumption that each cell has a single representative bed elevation. In practice, a 2 km² pentagon contains substantial internal topographic variability — ridges, channels, roads, and built structures — that controls whether and how water accumulates and routes. FloodA5 addresses this through Sub-Grid Sampling (SGS), which replaces the single cell elevation with a hypsometric curve: a pre-computed lookup table relating water surface elevation (WSE) to stored volume and wetted plan area.
