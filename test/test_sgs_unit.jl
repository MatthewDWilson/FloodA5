...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    function make_flat_sgs_table(z_min::Float64, z_max::Float64;
                                  ...
                                  edge_width::Float64 = 10.0,
                                  z_sill_offset::Float64 = 0.5)
        ...
        # Edge hydraulic curves for the R-A kernel.
        # Model: rectangular channel of width `edge_width` with bed at
        # z_min - z_sill_offset.  For each adjacency slot, at WSE = bins[k]:
        #   A_edge(k) = max(0, bins[k] - z_sill) × edge_width
        #   P_edge(k) = edge_width  (full width wet once above sill)
        z_sill_edge = z_min - z_sill_offset
        edge_area   = zeros(Float64, n_bins, 5)
        edge_perim  = zeros(Float64, n_bins, 5)
        for slot in 1:2   # slot 1 = left neighbour, slot 2 = right neighbour
            for k in 1:n_bins
                d = bins[k] - z_sill_edge
                if d > 0.0
                    edge_area[k,  slot] = d * edge_width
                    edge_perim[k, slot] = edge_width
                end
            end
        end

        return A5Grid.SGSTable(bins, vol_curve, area_curve, cell_area, z_min, z_max,
                               edge_area, edge_perim)
    end
