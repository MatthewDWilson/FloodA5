...
#   Animation using a fixed primary colour range:
#     julia --project=. visualisation/view_h5_output.jl sim.h5 \\
#         --animate \\
#         --colorrange 0.0 0.02
#
...
Resolve a CLI colourmap specification to an explicit Makie colour gradient.

A leading '-' requests the reversed colourmap.

This deliberately resolves the named colourmap to an actual colour vector

rather than returning Reverse(Symbol). The installed PlotUtils/Makie version

accepts Symbols and colour collections in cgrad(), but does not accept
...
function resolve_colormap(s::String)
    ...
    # Resolve the name first while it is still a Symbol. This allows Makie /

    # PlotUtils to perform the normal colourmap-name validation.

    cmap = Symbol(name)

    # Construct an explicit 256-colour gradient.
    ...
    # Return the actual colour collection, reversed when requested.

    return reversed ? reverse(colors) : colors
...
function parse_args(args)
    ...
    # Primary colourmap and optional user-specified primary colour range.
    ...
    while i <= length(args)
        ...
        elseif arg == "--time" && i < length(args)
            ...
            isnothing(time_req) &&
                ...
                error(
                    ...
                    "--time must be a number (seconds), got: $(args[i + 1])"
        ...
        elseif arg == "--frame-step" && i < length(args)
            ...
            isnothing(frame_step) &&
                ...
                error(
                    ...
                    "--frame-step must be an integer, got: $(args[i + 1])"
        ...
        elseif arg == "--colorrange" && i + 2 <= length(args)

            color_min = tryparse(Float64, args[i + 1])
            color_max = tryparse(Float64, args[i + 2])

            (isnothing(color_min) || isnothing(color_max)) &&
                error(
                    "--colorrange requires two numeric values: MIN MAX; " *
                    "got: $(args[i + 1]) $(args[i + 2])"
                )

            color_max > color_min ||
                error(
                    "--colorrange requires MAX > MIN; " *
                    "got: $color_min $color_max"
                )
...
function load_h5(h5file::String)
    ...
    h5open(h5file, "r") do f
        ...
        println(
            ...
            "  Duration: $(round(times[end] / 3600, digits=2)) h"
...
explicit colour-gradient lookup.

The colourmaps passed to this function are already resolved by

resolve_colormap(), so reversed colourmaps are represented as ordinary

colour collections rather than Reverse{Symbol}.
...
function animate_h5(
    ...
    mesh_df, mesh_idx =
        ...
        load_parquet(mesh_path)

    println(

        "  Mesh cells : $(nrow(mesh_df))"
    ...
    if isnothing(colorrange_override)

        primary_colorrange =
            animation_colorrange(
                var_data,
                var_label,
            )
        ...
        primary_colorrange = colorrange_override
...
function interactive_view(
    ...
    if isnothing(colorrange_override)
        ...
        finite_map_vals = map_vals[isfinite.(map_vals)]

        isempty(finite_map_vals) &&
            error(
                "Selected variable '$var_label' contains no finite values " *
                "in the selected frame"
            )

        vmin = minimum(finite_map_vals)
        vmax = maximum(finite_map_vals)
    ...
    if !isnothing(cell_idx)
        ...
        text!(
            ...
            text =
                ...
                "  $(cell_id[1:min(8, length(cell_id))])…",
...
if args.animate
    ...
    animate_h5(
        ...
        colorrange_override = args.colorrange,
